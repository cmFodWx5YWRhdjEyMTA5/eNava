...
import com.enavamaratha.enavamaratha.parser.RssAtomParser;
...
import com.enavamaratha.enavamaratha.provider.FeedData.TaskColumns;
import com.enavamaratha.enavamaratha.utils.ArticleTextExtractor;
...
public class FetcherService extends IntentService {
    ...
    public static final String ACTION_REFRESH_FEEDS = "com.enavamartha.enavamaratha.REFRESH";
    public static final String ACTION_MOBILIZE_FEEDS = "com.enavamartha.enavamaratha.MOBILIZE_FEEDS";
    public static final String ACTION_DOWNLOAD_IMAGES = "com.enavamartha.enavamaratha.DOWNLOAD_IMAGES";
    ...
    private int mGuidpos, mMainImgPos,titlepos;
    DatabaseHelper dbb;
    private SQLiteDatabase database;
    private ArrayList<String> topnotifcationtitle;
    private ArrayList<String> topnotificationimageurl;
    ...
    public void onHandleIntent(Intent intent) {
        ...
        { // == Constants.ACTION_REFRESH_FEEDS
            ...
            String[] cols = new String[]{EntryColumns.FEED_ID, EntryColumns.TITLE, EntryColumns.IMAGE_URL,EntryColumns.FLAG,EntryColumns.GUID};
            String res, guid ="";



            Cursor curso =database.query(EntryColumns.TABLE_NAME, cols, fil, null, null, null,orderBy, limit);
            //   Cursor curso=getContentResolver().query(EntryColumns.ENTRIES_FOR_FEED_CONTENT_URI(1), new String[]{EntryColumns.FEED_ID,EntryColumns.TITLE,EntryColumns.IMAGE_URL}, EntryColumns.WHERE_UNREAD,new String[]{ orderBy }, limit);
            titlepos = curso.getColumnIndex(EntryColumns.TITLE);
            mMainImgPos = curso.getColumnIndex(EntryColumns.IMAGE_URL);
            mGuidpos=curso.getColumnIndex(EntryColumns.GUID);

            int n =1;

            if (curso != null && curso.getCount() > 0 && curso.moveToFirst())
            {
                do
                {

                    res = curso.getString(titlepos);
                    guid=curso.getString(mGuidpos);
                    System.out.println("Title in Intent" + res);
                    String Imageuri = curso.getString(mMainImgPos);
                    System.out.println("Post Id" + guid);

                    // update entries table for set flag is yes
                    ...
                    if (newCount > 0)
                        if (PrefUtils.getBoolean(PrefUtils.NOTIFICATIONS_ENABLED, true))
                        {
                                ...
                                {
                                    Constants.NOTIF_MGR.notify(n, notifBuilder.getNotification());
                                }
        ...
        /*    if (newCount > 0)
            {
                if (PrefUtils.getBoolean(PrefUtils.NOTIFICATIONS_ENABLED, true))
                {
                    Cursor cursor = getContentResolver().query(EntryColumns.ENTRIES_FOR_FEED_CONTENT_URI(1), new String[]{Constants.DB_COUNT}, EntryColumns.WHERE_UNREAD, null, null);
    ...
    private void mobilizeAllEntries() {
        ...
        while (cursor.moveToNext())
        {
            ...
            {
                if (entryCursor.isNull(entryCursor.getColumnIndex(EntryColumns.MOBILIZED_HTML)))
                { // If we didn't already mobilized it
